...
$dataFile = __DIR__ . '/../data/players.json';
...
if ($method === 'GET') {
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        echo json_encode(['error' => 'Stored player data is invalid.'], JSON_UNESCAPED_UNICODE);
        exit;
    }
...
if ($method === 'POST') {
    ...
    if ($encoded === false || file_put_contents($dataFile, $encoded . PHP_EOL, LOCK_EX) === false) {
        http_response_code(500);
        echo json_encode(['error' => 'Could not persist player data.'], JSON_UNESCAPED_UNICODE);
        exit;
    }
